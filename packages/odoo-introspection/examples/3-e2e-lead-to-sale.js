...
async function main() {
    const client = new client_1.OdooClient(config);
    ...
    try {
        ...
        const keyLeadFields = leadFields.filter((f) => ['name', 'email_from', 'phone', 'partner_id', 'expected_revenue'].includes(f.name));
        ...
        const keyPartnerFields = partnerFields.filter((f) => ['name', 'email', 'phone', 'is_company', 'street', 'city'].includes(f.name));
        ...
        const keySaleFields = saleFields.filter((f) => ['name', 'partner_id', 'opportunity_id', 'state', 'amount_total'].includes(f.name));
        ...
        const [lead] = await client.read('crm.lead', [leadId], ['name', 'partner_id', 'stage_id', 'create_date']);
        ...
        const activityTypes = await client.searchRead('mail.activity.type', [], { fields: ['name', 'id'], limit: 5 });
        ...
        // Verify the update
        const [updatedLead] = await client.read('crm.lead', [leadId], ['partner_id']);
        ...
        const [quotation] = await client.read('sale.order', [saleOrderId], ['name', 'state', 'partner_id', 'opportunity_id', 'amount_total']);
        ...
        console.log(`   - Opportunity: ${quotation.opportunity_id ? quotation.opportunity_id[1] : 'None'}`);
        ...
        const [confirmedOrder] = await client.read('sale.order', [saleOrderId], ['name', 'state']);
 ...
 */
async function ensureModulesInstalled(client) {
 ...
 * - Delete partner last
 */
async function cleanup(client, records) {
...
main();
//# sourceMappingURL=3-e2e-lead-to-sale.js.map