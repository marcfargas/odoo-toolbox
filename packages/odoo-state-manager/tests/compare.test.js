...
describe('State Comparison Module', () => {
    describe('compareRecord - Primitive Fields', () => {
        it('detects no changes when states are identical', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { name: 'Q1 Planning', active: true }, { name: 'Q1 Planning', active: true });
            expect(changes).toHaveLength(0);
        });
        it('detects string field changes', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { name: 'Q1 Planning' }, { name: 'Q1' });
        ...
        it('detects numeric field changes', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { sequence: 10 }, { sequence: 5 });
            expect(changes).toHaveLength(1);
        ...
        it('detects boolean field changes', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { active: true }, { active: false });
            expect(changes).toHaveLength(1);
            expect(changes[0].newValue).toBe(true);
            expect(changes[0].oldValue).toBe(false);
        });
        it('detects new fields (not in actual state)', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { name: 'Q1 Planning' }, {});
            expect(changes).toHaveLength(1);
            expect(changes[0]).toEqual({
                path: 'name',
                operation: 'create',
        ...
        it('ignores fields in actual state not in desired', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { name: 'Q1 Planning' }, { name: 'Q1 Planning', description: 'Old description' });
    ...
    describe('compareRecord - Many2One Fields', () => {
        it('normalizes many2one [id, name] tuple from Odoo', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { project_id: 5 }, { project_id: [5, 'Q1 Planning'] } // Odoo format
            );
            expect(changes).toHaveLength(0); // No change - same ID
        });
        it('detects many2one changes despite name differences', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { project_id: 5 }, { project_id: [3, 'Q2 Planning'] } // Different ID, so it's a change
            );
            expect(changes).toHaveLength(1);
        ...
        it('handles null many2one values', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { manager_id: null }, { manager_id: [1, 'John Doe'] });
            expect(changes).toHaveLength(1);
            expect(changes[0].operation).toBe('update');
            expect(changes[0].newValue).toBe(null);
        });
        it('detects null to non-null many2one changes', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { manager_id: 5 }, { manager_id: null });
            expect(changes).toHaveLength(1);
            expect(changes[0].operation).toBe('update');
            expect(changes[0].newValue).toBe(5);
        ...
        it('uses field metadata to identify many2one types', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { project_id: 5 }, { project_id: [5, 'Project Name'] }, { fieldMetadata });
            expect(changes).toHaveLength(0);
        });
    });
    describe('compareRecord - One2Many/Many2Many Fields', () => {
        it('compares one2many as ID arrays', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { task_ids: [1, 2, 3] }, { task_ids: [1, 2, 3] });
            expect(changes).toHaveLength(0);
        });
        it('detects one2many changes regardless of order', () => {
            // Array comparison should handle order variations
            const changes = (0, compare_1.compareRecord)('project.project', 1, { task_ids: [1, 2, 3] }, { task_ids: [3, 2, 1] });
        ...
        it('detects one2many element additions', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { task_ids: [1, 2, 3] }, { task_ids: [1, 2] });
            expect(changes).toHaveLength(1);
            expect(changes[0].newValue).toEqual([1, 2, 3]);
        });
        it('handles empty one2many arrays', () => {
            const changes = (0, compare_1.compareRecord)('project.project', 1, { task_ids: [] }, { task_ids: [1, 2] });
            expect(changes).toHaveLength(1);
    ...
    describe('compareRecord - Field Metadata and Computed Fields', () => {
        it('skips readonly fields', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { create_date: '2024-01-01' }, { create_date: '2024-01-02' }, { fieldMetadata });
        ...
        it('skips computed fields', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { progress: 50.0 }, { progress: 25.0 }, { fieldMetadata });
        ...
        it('includes writable fields not marked as readonly/computed', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { name: 'New Name' }, { name: 'Old Name' }, { fieldMetadata });
            expect(changes).toHaveLength(1);
        });
    });
    describe('compareRecord - Custom Comparators', () => {
        it('uses custom comparator when provided', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { tags: 'urgent, important' }, { tags: 'URGENT, IMPORTANT' }, { customComparators });
        ...
        it('custom comparator detects actual changes', () => {
            ...
            const changes = (0, compare_1.compareRecord)('project.task', 1, { tags: 'urgent' }, { tags: 'important' }, { customComparators });
            expect(changes).toHaveLength(1);
        });
    });
    describe('compareRecords - Multiple Records', () => {
        it('compares multiple records and returns diffs for changed ones', () => {
            ...
            const diffs = (0, compare_1.compareRecords)('project.task', desiredStates, actualStates);
    ...
    describe('Edge Cases', () => {
        it('handles null values correctly', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { notes: null }, { notes: 'Some text' });
            expect(changes).toHaveLength(1);
            expect(changes[0].newValue).toBe(null);
        });
        it('handles undefined in actual state as null', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { notes: 'Text' }, {});
            expect(changes).toHaveLength(1);
            expect(changes[0].oldValue).toBe(null);
        });
        it('detects changes in nested objects', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, { metadata: { status: 'active' } }, { metadata: { status: 'inactive' } });
            expect(changes).toHaveLength(1);
        });
        it('handles empty object comparison', () => {
            const changes = (0, compare_1.compareRecord)('project.task', 1, {}, {});
            expect(changes).toHaveLength(0);
        });
    });
});
